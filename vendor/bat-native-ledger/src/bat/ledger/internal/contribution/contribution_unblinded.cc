...
bool HasTokenExpired(ledger::UnblindedToken token) {
...
bool GetStatisticalVotingWinner(
    double dart,
    const double amount,
    std::vector<ledger::ContributionPublisher> list,
...
void Unblinded::TokenProcessed(
    ...
    const std::string& publisher_key) {
  if (result == ledger::Result::LEDGER_OK) {
    // TODO update publisher amount in contribution
  }
...
void Unblinded::SetTimer(uint32_t* timer_id, uint64_t start_timer_in) {
  if (!timer_id) {
    return;
  }
