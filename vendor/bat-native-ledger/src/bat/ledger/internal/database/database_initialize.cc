...
void DatabaseInitialize::OnInitialize(
  ...
  if (!response->result ||
      response->result->get_value()->which() != ledger::DBValue::Tag::INT_VALUE) {
    callback(ledger::Result::DATABASE_INIT_FAILED);
    return;
  }

  const auto current_table_version = response->result->get_value()->get_int_value();
...
void DatabaseInitialize::EnsureCurrentVersion(
    ...
    ledger::ResultCallback callback) {
  BLOG(ledger_, ledger::LogLevel::LOG_ERROR) << "EnsureCurrentVersion - table version: " << table_version;
